...
if __name__ == '__main__':
    ...
    GT_dir = cli.GT_dir if cli.GT_dir else eval_cfg.get('gt_dir', '/.../levir_CD/label/')
    pred_dir = cli.pred_dir if cli.pred_dir else eval_cfg.get('pred_dir', '/.../SAM_CD/eval/Levir_CD/SAM_CD/')
    ...
    for idx, it in enumerate(data_list):
        if it.lower().endswith(('.png', '.jpg', '.jpeg')): num_valid+=1
    ...
    for idx, it in enumerate(data_list):
        if it.lower().endswith(('.png', '.jpg', '.jpeg')):
            ...
            h,w = pred.shape[:2]
            #pred = pred[:,:,0]/255.0
            GT_path = os.path.join(GT_dir, os.path.splitext(it)[0] + '.png')
