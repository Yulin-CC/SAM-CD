...
class BaseModel(torch.nn.Module):
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        max_idx = max(embed)
        if 1 not in self.save:
            self.save.append(1)  # save stride-4 features for SAM-CD multi-scale extraction
        for m in self.model:
            if isinstance(m, Segment):
                m.f = [15, 18, 21, 1]  # feed stride-4 (layer 1) as 4th input to Segment head
