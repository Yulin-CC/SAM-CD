...
# SAM_CD_TBR_PATCH — Segment Change Detection 四层特征补丁（推理 + ONNX 导出）
SAM_CD_TBR_PATCH_VERSION = "1.0"
...
class Segment(Detect):
    ...
    def forward(self, x: list[torch.Tensor]) -> tuple | list[torch.Tensor] | dict[str, torch.Tensor]:
        ...
        ms_feats = list(x)  # capture all input feature maps (may include stride-4 from layer 1)
        x = x[:3]  # use only the standard 3 scales for detection/segmentation
        ...
        if self.training:
            return preds
        # SAM-CD: export 仅输出四层 ms_feats（feat_l0~l3），跳过 detect/proto
        if self.export:
            return tuple(ms_feats)
        return ((outputs[0], proto), preds, ms_feats)
